    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 p-6">
      ...
      {showToast && user && (
        <div className="fixed top-5 right-5 z-50 animate-slide-in">
          <div className="bg-white/10 backdrop-blur-md border border-white/20 rounded-2xl px-5 py-4 shadow-2xl flex items-center gap-3 min-w-[260px]">
            ...
            <div>
              <p className="text-white font-semibold text-sm">Login successful!</p>
              <p className="text-purple-300 text-xs mt-0.5">Welcome, {user.full_name}! 👋</p>
            </div>
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
          {[
            { icon: '🔍', label: 'Browse Providers', desc: 'Find service providers and book appointments' },
            { icon: '📅', label: 'My Appointments', desc: 'View and manage your bookings' },
          ].map((card) => (
            <div key={card.label}
        ...
        <div className="bg-green-500/20 border border-green-500/40 rounded-2xl p-5 text-center">
          <p className="text-green-300 font-semibold text-lg">✅ You are logged in as a Customer!</p>
          <p className="text-green-200/70 text-sm mt-1">
            Full features coming in Day 4 & 5.
          </p>
        </div>
      </div>
    </div>
